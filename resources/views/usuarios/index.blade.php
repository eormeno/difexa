                ...
                @forelse ($usuarios as $usuario)
                  <div class="rounded-xl bg-gray-300 shadow p-2">
                    <div class="group relative">
                      ...
                      <button class="hidden group-hover:block cursor-pointer absolute top-0 right-7 p-2 text-gray-400 hover:text-green-500" x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-usuario-verificado-{{ $usuario->id }}')">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                            <path fill-rule="evenodd" d="M19.916 4.626a.75.75 0 01.208 1.04l-9 13.5a.75.75 0 01-1.154.114l-6-6a.75.75 0 011.06-1.06l5.353 5.353 8.493-12.739a.75.75 0 011.04-.208z" clip-rule="evenodd" />
                        </svg>
                      </button>

                      <button class="hidden group-hover:block cursor-pointer absolute top-0 right-0 p-2 text-gray-400 hover:text-red-500" x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-usuario-deletion-{{ $usuario->id }}')">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
