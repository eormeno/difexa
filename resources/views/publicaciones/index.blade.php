                            ...
                            @forelse ($publicaciones as $publicacion)
                                <div class="rounded-xl bg-gray-300 shadow p-2">
                                    <div class="group relative">
                                        ...
                                        <button class="hidden group-hover:block cursor-pointer absolute top-0 right-0 p-2 text-red-400 hover:text-red-500" x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirmar-eliminacion-{{$publicacion->id}}')">
                                ...
                                <x-modal name="confirmar-eliminacion-{{$publicacion->id}}" :show="$errors->userDeletion->isNotEmpty()" focusable>
                                    <form method="post" action="{{ route('publicaciones.destroy', $publicacion) }}" class="p-6">
                                        ...
                                        <h1 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                                            {{ __('Se eliminará la publicacion') }} <span class="font-extrabold"> {{ $publicacion->titulo }}</span>
                                        </h1>
