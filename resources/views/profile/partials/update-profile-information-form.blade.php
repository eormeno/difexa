<section>
    ...
    @if (session('nuevoTema'))
        <x-modal name="confirmar-eliminacion-{{session('nuevoTema')->id}}" :show="True" focusable>
            <form method="post" action="{{ route('profile.cambiarTema', session('nuevoTema')) }}" class="p-6">
            @csrf
            @method('patch')
            <h1 class="text-lg font-medium text-gray-900 dark:text-gray-100 m-5">
                {{ __("Se cambiara del tema ") }}{{$user->tema->titulo}} al tema <span class="font-extrabold">{{session('nuevoTema')->titulo}}</span>
            </h1>
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                {{ __('¿Está seguro?') }}
            </h2>
            <p class="text-md font-medium text-gray-800 dark:text-gray-100 m-5">
                {{ __('Si modifica el tema, debera verificarse de nuevo en la FCEFN.') }}
            </p>
    ...
    @if (session('status') === 'profile-updated')
        <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 5000)"
            class="text-lg text-green-500 dark:text-gray-400">{{ __('Información actualizada.') }}</p>
    @endif
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="documento" name="documento" type="number" class="mt-1 block w-full" :value="old('documento', $user->documento)"
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && !$user->hasVerifiedEmail())
                <div>
                    <p class="text-sm mt-2 text-gray-800 dark:text-gray-200">
                        {{ __('Su email no ha sido verificado.') }}

                        <button form="send-verification"
                            class="underline text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800">
                            {{ __('Presione aquí para reenviar el email de verificación.') }}
                        </button>
                    </p>
